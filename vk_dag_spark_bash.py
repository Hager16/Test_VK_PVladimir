...
with DAG(
    ...
    daily_agg = BashOperator(
        ...
        bash_command="""
        source /opt/airflow/venv/bin/activate && spark-submit --master local /opt/airflow/scripts/day_agg_spark.py /opt/airflow/intermediate_spark /opt/airflow/input_spark {{ execution_date.subtract(days=1).strftime("%Y-%m-%d") }} && deactivate
        """,
        dag=dag
    )
    ...
    weekly_agg = BashOperator(
        ...
        bash_command="""
        source /opt/airflow/venv/bin/activate && spark-submit --master local /opt/airflow/scripts/week_agg_spark.py /opt/airflow/intermediate_spark /opt/airflow/output_spark {{ ds }} && deactivate
        """,
        dag=dag
    )
